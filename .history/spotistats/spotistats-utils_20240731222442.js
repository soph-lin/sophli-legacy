...
function getYesterday() { // Return yesterday's date set at midnight
    const yesterday = getToday().setDate(today.getDate() - 1);
...
function getLastWeek() { // Return last week, starting date of Sunday and ending date of Saturday
    ...
    const lastSaturday = today.setDate(today.getDate() - 7 - dayOfWeek);
    const lastSunday = new Date(lastSaturday.valueOf()).setDate(lastSaturday.getDate() - dayOfWeek);
}
