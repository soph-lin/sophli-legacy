class Sun {
    ...
    cycle(startTime) {
        if (this.moving) {
            return `Cycle ongoing, can't start new cycle`;
        }
        ...
        // if didn't reach end, continue cycle
...
function setupDayGlobe() {
    ...
    if (isDaytime(sunriseHr, sunsetHr)) {
        ...
        window.addEventListener('blur', () => { // stop cycle if inactive tab
            console.log('blur');
            clearInterval(cycleID);
            sun.moving = false;
