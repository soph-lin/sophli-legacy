class Sun {
    ...
    cycle(startTime) {
        ...
        if (startTime > this.cycleTime) {
            console.log('cycle already done');
            return;
        }

        // if didn't reach end, start cycle
        const self = this;  // this in setTimeout is evaluated in different context, so need to pass self explicitly
        ...
        window.addEventListener('blur', () => { // stop cycle if leave tab
            ...
            window.addEventListener('focus', () => { // continue cycle
                ...
                if (testShortCycle) {
                    ...
                    timeLapsed = timeSinceSunrise(sunriseHr);
                }
