...
function bindEvents() {
    ...
    if (document.querySelector('.dropInfo')) {
        document.addEventListener('click', function(e) {
            console.log(e.target);
            if (e.target.classList.contains('dropInfo') && e.target.classList.contains('header')) {
                console.log('drop!');
            }
        });
    }
    const dropInfoEls = document.querySelector('.dropInfo');

    dropInfoEls.forEach(el => el.addEventListener('click', event => {
        console.log();
    }));
}