...
function turnOnLight(makeOn, customPullCordHandle) { // change appearance of lightbulb icon
    ...
    if (customPullCordHandle) {
        splashMsg = customPullCordHandle.randMsg();
    }
    else if (makeOn) {
        splashMsg = random(lightModeMessages);
    }
    else {
        splashMsg = random(darkModeMessages);
    }
...
class PullCordHandle {
    constructor(icon, msgs, params) {
        ...
        this.msgs = msgs;
        if (params) {
            this.sound = params.sound ? sound : null;
            this.customCoords = params.customCoords ? params.customCoords : null;
        }
    }
...
function setupPullCordHandle() {
    const specialHandles = {
        '2-14': new PullCordHandle('🌹', 'a rose for thee', {customCoords: {x: -7}}),
        '12-25': new PullCordHandle('🎁', 'a gift for you on a special day 🎄')
    };
    const miscHandles = [
        new PullCordHandle('🕷️', 'aaah! spider'),
        new PullCordHandle('⚓', 'ahoy matey!', {customCoords: {x: -11}}),
    ...
    if (customPullCordHandle) {
        ...
        const customCoords = customPullCordHandle.customCoords;
        if (customCoords) {
            if (customCoords.x) customPullCordHandleEl.setAttribute('x', customCoords.x);
            if (customCoords.y) customPullCordHandleEl.setAttribute('y', customCoords.y);
        }
    }
